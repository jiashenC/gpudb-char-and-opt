...
template <int BLOCK_THREADS, int ITEMS_PER_THREAD>
__global__ void projectSigmoid(float *in1, float *in2, float *out,
                               int num_items) {
  float items[ITEMS_PER_THREAD];
  float items2[ITEMS_PER_THREAD];
  float res[ITEMS_PER_THREAD];
...
float projectGPU(float *in1, float *in2, float *out, int num_items) {
  ...
  TIME_FUNC((project<128, 4><<<num_blocks, 128>>>(in1, in2, out, num_items)),
            time_proj);
...
float projectSigmoidGPU(float *in1, float *in2, float *out, int num_items) {
  ...
  int num_blocks = (num_items + tile_items - 1) / tile_items;
  TIME_FUNC(
      (projectSigmoid<128, 4><<<num_blocks, 128>>>(in1, in2, out, num_items)),
      time_proj);
...
int main(int argc, char **argv) {
  ...
  float *d_in1 = NULL;
  CubDebugExit(
      g_allocator.DeviceAllocate((void **)&d_in1, sizeof(float) * num_items));

  float *d_in2 = NULL;
  CubDebugExit(
      g_allocator.DeviceAllocate((void **)&d_in2, sizeof(float) * num_items));
  ...
  projectGPU(d_in1, d_in2, d_out, num_items);
  ...
  // Cleanup
  if (d_in1)
    CubDebugExit(g_allocator.DeviceFree(d_in1));
  if (d_in2)
    CubDebugExit(g_allocator.DeviceFree(d_in2));
  if (d_out)
