...
__global__ void probe(int *lo_orderdate, int *lo_partkey, int *lo_custkey,
  ...
  if (LOAD_TYPE != 0) {
    Term<int, BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags);
  }

  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(lo_custkey + tile_offset,
                                                  items, num_tile_items);
  ...
  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(lo_partkey + tile_offset,
                                                  items, num_tile_items);
  BlockProbeAndPHT_1<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
  ...
  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(lo_revenue + tile_offset,
                                                  revenue, num_tile_items);
  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(lo_supplycost + tile_offset,
                                                  items, num_tile_items);
...
float runQuery(int *lo_orderdate, int *lo_custkey, int *lo_partkey,
  ...
  // CubDebugExit(
  //     cudaMemcpy(s_res, ht_s, s_len * 2 * sizeof(int),
  ...
  // CubDebugExit(
  //     cudaMemcpy(c_res, ht_c, c_len * 2 * sizeof(int),
  //     cudaMemcpyDeviceToHost));
  ...
  // CubDebugExit(
  //     cudaMemcpy(p_res, ht_p, p_len * 2 * sizeof(int),
  //     cudaMemcpyDeviceToHost));
  ...
  //   CubDebugExit(cudaMemcpy(h_ht_s, ht_s, 2 * s_len * sizeof(int),
  //   cudaMemcpyDeviceToHost)); for (int i=0; i<s_len; i++) if (h_ht_s[i*2] !=
  //   0) num_s += 1;

  //   cout << "Num Matched" << " " << num_s << " " << s_len << endl;

  //   CubDebugExit(cudaMemcpy(h_ht_d, ht_d, 2 * d_val_len * sizeof(int),
  //   cudaMemcpyDeviceToHost)); for (int i=0; i<d_val_len; i++) if (h_ht_d[i*2]
  //   != 0) num_d += 1;

  //   cout << "Num Matched" << " " << num_d << " " << d_len << endl;

  //   CubDebugExit(cudaMemcpy(h_ht_c, ht_c, 2 * c_len * sizeof(int),
  //   cudaMemcpyDeviceToHost)); for (int i=0; i<c_len; i++) if (h_ht_c[i*2] !=
  //   0) num_c += 1;

  //   cout << "Num Matched" << " " << num_c << " " << c_len << endl;

  //   CubDebugExit(cudaMemcpy(h_ht_p, ht_p, 2 * p_len * sizeof(int),
  //   cudaMemcpyDeviceToHost)); for (int i=0; i<p_len; i++) if (h_ht_p[i*2] !=
  //   0) num_p += 1;
