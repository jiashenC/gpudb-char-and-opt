...
float runQuery(int *lo_orderdate, int *lo_custkey, int *lo_partkey,
  ...
  //   if (h_res[4 * i] != 0) {
  //     cout << h_res[4 * i] << " " << h_res[4 * i + 1] << " " << h_res[4 * i +
  //     2]
  //          << " " << h_res[4 * i + 3] << endl;
  //     res_count += 1;
  //   }
...
int main(int argc, char **argv) {
  ...
  runQuery(d_lo_orderdate, d_lo_custkey, d_lo_partkey, d_lo_suppkey,
           ...
           d_p_partkey, d_p_category, d_p_brand1, P_LEN, d_s_suppkey,
           d_s_nation, d_s_city, S_LEN, d_c_custkey, d_c_region, C_LEN,
           g_allocator);
  finish = chrono::high_resolution_clock::now();
  std::chrono::duration<double> diff = finish - st;
  ...
  for (int t = 0; t < num_trials - 1; t++) {
    ...
    runQuery(d_lo_orderdate, d_lo_custkey, d_lo_partkey, d_lo_suppkey,
             ...
             D_LEN, d_p_partkey, d_p_category, d_p_brand1, P_LEN, d_s_suppkey,
             d_s_nation, d_s_city, S_LEN, d_c_custkey, d_c_region, C_LEN,
             g_allocator);
    finish = chrono::high_resolution_clock::now();
    diff = finish - st;
