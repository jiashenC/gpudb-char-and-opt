                           ...
                           int lo_num_entries, unsigned long long *revenue)
{
  ...
  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(lo_quantity + tile_offset,
                                                  items, num_tile_items);
  BlockPredAndLT<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 25, selection_flags, num_tile_items);

  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(lo_discount + tile_offset,
                                                  items, num_tile_items);
  ...
  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      lo_extendedprice + tile_offset, items2, num_tile_items);
