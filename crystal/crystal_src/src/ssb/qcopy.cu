...
float runQuery(int *lo_orderdate, int *lo_partkey, int *lo_suppkey,
  ...
  int tile_items = 128 * 8;

  build_hashtable_s<128, 8><<<(s_len + tile_items - 1) / tile_items, 128>>>(
      s_region, s_suppkey, s_len, ht_s, s_len);
  /*CHECK_ERROR();*/

  build_hashtable_p<128, 8><<<(p_len + tile_items - 1) / tile_items, 128>>>(
      p_category, p_partkey, p_brand1, p_len, ht_p, p_len);
  /*CHECK_ERROR();*/

  int d_val_min = 19920101;
  build_hashtable_d<128, 8><<<(d_len + tile_items - 1) / tile_items, 128>>>(
  ...
  for (int i = 0; i < 1000; i++) {
    probe<128, 8><<<(lo_len + tile_items - 1) / tile_items, 128, 0,
                    streams[i % num_stream]>>>(
        lo_orderdate, lo_partkey, lo_suppkey, lo_revenue, lo_len, ht_s, s_len,
        ht_p, p_len, ht_d, d_val_len, res);
    cudaMemcpyAsync(h_res, res, res_array_size * sizeof(int),
                    cudaMemcpyDeviceToHost, streams[i % num_stream]);
  }
...
int main(int argc, char **argv) {
  ...
  runQuery(d_lo_orderdate, d_lo_partkey, d_lo_suppkey, d_lo_revenue, LO_LEN,
           d_p_partkey, d_p_brand1, d_p_category, P_LEN, d_d_datekey, d_d_year,
  ...
  for (int t = 0; t < num_trials - 1; t++) {
    ...
    st = chrono::high_resolution_clock::now();
    runQuery(d_lo_orderdate, d_lo_partkey, d_lo_suppkey, d_lo_revenue, LO_LEN,
             d_p_partkey, d_p_brand1, d_p_category, P_LEN, d_d_datekey,
             d_d_year, D_LEN, d_s_suppkey, d_s_region, S_LEN, g_allocator);
    finish = chrono::high_resolution_clock::now();
    diff = finish - st;
    cout << "Execution time: " << diff.count() * 1000 << " ms" << endl;
    nsys_detach(trial, num_trials);
