...
* int       (*verify) ();    -- function to verfiy the data set without building
* it
...
void stop_proc(int signum) { exit(0); }
...
int set_files(int i, int pload) {
  ...
  child_table : {
    if (pload != -1)
      ...
      printf("Enter new destination for %s data: ", tdefs[i].name);
...
int partial(int tbl, int s) {
  ...
  if (verbose > 0) {
    fprintf(stderr, "\tStarting to load stage %d of %d for %s...", s, children,
            tdefs[tbl].comment);
  }
...
int pload(int tbl) {
  ...
  if (verbose > 0) {
    fprintf(stderr, "Starting %d children to load %s", children,
            tdefs[tbl].comment);
  }
...
int main(int ac, char **av) {
...
#else
  tdefs[ORDER].base *= ORDERS_PER_CUST;      /* have to do this after init */
  tdefs[LINE].base *= ORDERS_PER_CUST;       /* have to do this after init */
  tdefs[ORDER_LINE].base *= ORDERS_PER_CUST; /* have to do this after init */
#endif
  fnames = 0;
  db_name = NULL;
  gen_sql = 0;
  ...
  */
  if (updates) {
    ...
    fix1 = (double)tdefs[LINE].base /
           (double)10000; /*represent the %% percentage (n/100)%*/
#else
    ...
    while (upd_num < updates) {
      if (verbose > 0)
#ifdef SSBM
        fprintf(stderr, "Generating update pair #%d for %s [pid: %d]",
