...
int dbg_print(int format, FILE *target, void *data, int len, int sep) {
  ...
  case DT_VSTR:
    ...
    fprintf(target, "%c%c%-*s", (len >> 8) & 0xFF, len & 0xFF, len,
            (char *)data);
    break;
#endif /* MVS */
  case DT_INT:
  ...
  case DT_HUGE:
    ...
    if (*(long *)((long *)data + 1) == 0)
      ...
      fprintf(target, "%5ld%07ld", *(long *)((long *)data + 1), *(long *)data);
    else
      fprintf(target, "%ld%07ld", *(long *)((long *)data + 1), *(long *)data);
#else
...
int pr_line(order_t *o, int mode) {
  ...
  for (i = 0; i < o->lines; i++) {
    ...
    PR_VSTR_LAST(fp_l, o->l[i].comment,
                 (columnar) ? (long)(ceil(L_CMNT_LEN * V_STR_HGH))
                            : o->l[i].clen);
    PR_END(fp_l);
  }
...
int pr_part(part_t *part, int mode) {
  ...
  PR_VSTR(p_fp, part->name, (columnar) ? (long)P_NAME_LEN : part->nlen);
  ...
  PR_VSTR(p_fp, part->type, (columnar) ? (long)P_TYPE_LEN : part->tlen);
...
int pr_psupp(part_t *part, int mode) {
  ...
  for (i = 0; i < SUPP_PER_PART; i++) {
    ...
    PR_VSTR_LAST(ps_fp, part->s[i].comment,
                 (columnar) ? (long)(ceil(PS_CMNT_LEN * V_STR_HGH))
                            : part->s[i].clen);
    PR_END(ps_fp);
  }
...
/*SSBM don't have partsupplier table*/
#else
int pr_part_psupp(part_t *part, int mode) {
...
int pr_drange(int tbl, long min, long cnt, long num) {
  ...
  for (child = min; cnt > 0; child++, cnt--) {
    ...
    if (gen_sql) {
      fprintf(dfp, "delete from %s where %s between %ld and %ld;\n",
...
int pr_date(date_t *d, int mode) {
  ...
  PR_VSTR(d_fp, d->sellingseason, (columnar) ? (long)D_SEASON_LEN : d->slen);
...
#endif
/*
 * verify functions: routines which replace the pr_routines and generate a
 * pseudo checksum
 * instead of generating the actual contents of the tables. Meant to allow large
 * scale data
...
#ifdef SSBM
#else
int vrf_psupp(part_t *part, int mode) {
  long i;
...
#ifdef SSBM
#else
int vrf_part_psupp(part_t *part, int mode) {
  vrf_part(part, mode);
  vrf_psupp(part, mode);
